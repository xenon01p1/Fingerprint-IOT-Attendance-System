...
async function handleIotResult(data) {
    switch (data.type) {
...
async function handleRegisterSuccess(data) {
    ...
    const fingerprint = await fingerprintRepo.findByDeviceAndIndex(deviceId, fingerprintIndex);
    if (!fingerprint) {
        broadcast({
...
async function publishAttendanceFeedback(deviceId, payload) {
